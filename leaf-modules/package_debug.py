...
with ZipFile(a.output,'w',ZIP_STORED) as z:
 ...
 if a.full_debug:
  ...
  readme='Release debug archive. The minimal freeroam SCM starts a new game in a clean test scene. F10 toggles first person on foot and in vehicles; mouse look remains active and the normal freecam setting is preserved. The local player head is hidden while looking through the first-person camera. Remove debug.leaf to restore the campaign SCM.\n'

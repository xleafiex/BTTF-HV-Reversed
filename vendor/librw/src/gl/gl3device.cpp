...
rasterRenderFast(Raster *raster, int32 x, int32 y)
{
	...
	case Raster::CAMERATEXTURE:
		...
		case Raster::CAMERA:
		{
			// Resolve multisampled scene colour before sampling it in droplets.
			// CopyTexSubImage cannot resolve an MSAA read framebuffer.
			...
			glBindFramebuffer(GL_READ_FRAMEBUFFER, oldDraw);
			glBindFramebuffer(GL_DRAW_FRAMEBUFFER, copyFbo);
			glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, natdst->texid, 0);
			GLboolean scissor = glIsEnabled(GL_SCISSOR_TEST);
			glDisable(GL_SCISSOR_TEST);
			glBlitFramebuffer(0, 0, src->width, src->height,
				x, dst->height-src->height-y, x+src->width, dst->height-y, GL_COLOR_BUFFER_BIT, GL_NEAREST);
